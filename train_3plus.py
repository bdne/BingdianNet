...
from Net.UNet3Plus.UNet3Plus import UNet_3Plus
...
class Segmentation(object):
    def __init__(self):
        ...
        self.save_model_name='ISBI_classification1.pth'
        #定义模型
        self.cuda=True
        self.device = torch.device("cuda" if self.cuda else "cpu")
        self.model=UNet_3Plus(1,2)#n_classes=2 前景概率和背景概率
    ...
    def test(self):
        ...
        if dice_acc > self.best_dice:
            torch.save(self.model.state_dict(), self.save_folder + 'UNet3plus\\'+self.save_model_name)
